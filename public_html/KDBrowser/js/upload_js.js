/*
	Hybrid JS formed with elements mootools-1.2-core.js, Swiff.Uploader.js, FancyUpload2.js, and Fx.ProgressBar.js
	Licensing information left intact
*/
...
//        alert("CREATE - " + arguments.callee.caller);
...
//        alert("INIT! " + arguments.callee.caller);
...
Swiff.Uploader = new Class({
	...
	initialize: function(options){
		...
		this.box = new Element('div', {
			styles: {
                                'z-index':10000000, // added to ensure this div gets placed in front of everything else, particularly with extjs
				position: 'absolute',
				overflow: 'hidden',
				height: 10, width: 10,
	...
	load: function(){
        debugger;
//                alert("LOAD " + arguments.callee.caller);
...
/**************************************************************
FancyUpload2.js (modified to FancyUpload3 for ExtJS)
**************************************************************/
...
var FancyUpload3 = new Class({
 ...
 //       alert('initialize');
	...
	onBeforeOpen: function(file, options) {
                fileProgressBar.updateText('Initialize upload for ' + file.name);
	...
	onOpen: function(file, overall) {
                fileProgressBar.updateText('Starting upload for ' + file.name);
	...
	onProgress: function(file, current, overall) {
                ...
                uploadProgressBar.updateProgress((overall.bytesLoaded / overall.bytesTotal), 'Uploading file - rate: ' + ((current.rate) ? this.sizeToKB(current.rate) : '- B') + ' Time Left: ' + Date.fancyDuration(current.timeLeft || 0));
                fileProgressBar.updateProgress((current.bytesLoaded / current.bytesTotal), 'Uploading file: ' + file.name);
...
//        alert('onSelect');
        	var errors = [];
                ...
                if (this.options.limitSize && (file.size > this.options.limitSize)) {
                    errors.push('size');
                    errString = "File Too Big (limit: " + this.sizeToKB(this.options.limitSize) + ")";
                }
		...
		if (errors.length) {
			...
			if (fn) fn.call(this, file, errors);
                        if (errString != "")
                        {
                            var p = new FileUploadEntry({
                                fileName: file.name,
                                fileSize: this.sizeToKB(file.size),
                                uploadStatus: errString
                            });
                            uploadGrid.store.add(p); // hack - this should work by this.list.store
                        }                        
	...
	onAllSelect: function(files, current, overall) {
                uploadProgressBar.updateProgress(0, 'Added ' + files.length + ' files.  Total Size: ' + this.sizeToKB(current.bytesTotal) + '.');
...
//        alert('onComplete');
                var recordIndex = uploadGrid.store.find("fileName",file.name);
                var recordItem = uploadGrid.store.getAt(recordIndex);
                recordItem.set('uploadStatus', 'Uploaded');
                fileProgressBar.updateText('Completed upload for ' + file.name);
...
//        alert('onError');
                var recordIndex = uploadGrid.store.find("fileName",file.name);
                var recordItem = uploadGrid.store.getAt(recordIndex);
                recordItem.set('uploadStatus', 'Failed: ' + error);
...
refreshGrid();
                ...
                uploadProgressBar.updateProgress(0, 'Upload Complete. Total Transfer: ' + this.sizeToKB(current.bytesTotal));
...
//        alert('browse');
	...
	upload: function(options) {
//        alert('upload ' + this.files.length);
//        debugger;
...
//			this.overallProgress.set(0);
                        uploadProgressBar.updateProgress(0, 'Upload started...');
		}
	},
...
//		this.overallTitle.set('html', 'Overall Progress (' + this.sizeToKB(bytesTotal) + ')');
                //if (undefined != files) < -- hack - this shouls work
                try {
                    uploadProgressBar.updateProgress(0, 'Added ' + files.length + ' files.  Total Size: ' + this.sizeToKB(current.bytesTotal) + 'kb).');
                }
                catch (ex) {
                    uploadProgressBar.updateProgress(0, 'Select Files...');
                }
	},
