...
function swfUploadLoaded() {
	...
	document.getElementById("btnCommit").onclick = function () { self.startUpload(); };
}
...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		if (numFilesSelected > 0) {
			...
			document.getElementById(this.customSettings.commitButtonId).disabled = false;
		}
...
function uploadSuccess(file, serverData) {
	try {
            ...
            if (response.error=='true') {
                if (response.errcode)
                    this.uploadError(file,response.errcode,response.message);
                else
                    this.uploadError(file,-250,response.message);
                return
            }
...
function uploadError(file, errorCode, message) {
	try {
		...
		case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
			progress.setStatus("Upload Failed: " + message);
		...
		case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
			...
			if (this.getStats().files_queued === 0) {
				...
				document.getElementById(this.customSettings.commitButtonId).disabled = true;
			}
